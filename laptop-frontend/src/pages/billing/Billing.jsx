...
export default function Billing() {
  ...
  useEffect(() => {
    ...
    api.get("/product/get.php", {
      ...
      if (r.data.status) {
        // Products are already filtered by active_status=1 from API
        // Filter only in-stock products for billing
        const activeProducts = (r.data.data || []).filter(
          p => p.stock > 0
        );
  ...
  const handleNameSearch = (value) => {
    ...
    nameSearchTimer.current = setTimeout(async () => {
      ...
      try {
        // Using admin_id instead of company_id
  ...
  const handleSearch = (value, index) => {
    ...
    // Products already filtered from API, just check stock and name
