...
export default function Dashboard() {
  ...
  const fetchStats = async (companyId) => {
    try {
      ...
      if (basic.data.status && analytics.data.status) {
        setStats({ 
          total_sales: basic.data.data.total_sales || 0, 
          total_products: basic.data.data.total_products || 0, 
  ...
  // FIXED: This is the main fix - use active_status instead of status
  const fetchLowStockProducts = async (companyId) => {
    try {
      const res = await api.get(`/product/get.php?company_id=${companyId}`);
      console.log("Product API Response:", res.data); // Debug
      
      if (res.data.status) {
        const products = res.data.data || [];
        console.log("All products:", products); // Debug
        
        // Filter: active_status = 1 AND stock <= 5
        const lowStock = products.filter(p => {
          const stock = Number(p.stock);
          const isActive = p.active_status === 1 || p.active_status === "1";
          return isActive && !isNaN(stock) && stock <= 5 && stock >= 0;
        });
        
        console.log("Low stock products:", lowStock); // Debug
  ...
  const fetchCreditDashboard = async (companyId) => {
    ...
    } catch (e) { console.error("Credit dashboard error:", e); }
  };
  ...
  return (
    <div className="db" style={{ fontFamily: font }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, animation: "db-up .35s ease both", flexWrap: "wrap", gap: 14, overflow: "visible", position: "relative", zIndex: 100 }}>
        <div>
              ...
              <tbody>
                {creditList.length > 0 ? creditList.map((c, i) => (
                  <tr key={i} className="db-row" style={{ borderBottom: "1px solid #fafafa" }}>
                    ...
                    <td style={tdStyle()}>{c.due_date || "N/A"}</td>
                    <td style={tdStyle()}>
                      <span style={{
